...
Output Format:
The first line contains the number of groups possible among the N string values as per the given condition.
...
4
-------------------------------------solution----------------------------------------
def count_groups(N, strings):
    groups = {}
    for string in strings:
        chars = ''.join(sorted(set(string)))
        groups[chars] = groups.get(chars, 0) + 1
    return len(groups)
